...
#immediately takes all the sql queries in the products variable
...
#uses the session method, also directly takes queries and puts it in variable
@views.route('/profile')
@cross_origin()
def profile():
    try:
...
#directly takes queries and puts it in variable
@views.route('/order')
@cross_origin()
def order():
    try:
